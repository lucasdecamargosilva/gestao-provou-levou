...
function mpCategoria(p) {
    ...
    if ((p.description || '').trim()) return 'link';
    return 'pix';
}

function mpDataISO(p) {
    return String(p.date_approved || p.date_created || '').slice(0, 10);
}

// Sugere o cliente: e-mail do pagador primeiro, depois valor único entre os planos
function mpSugerirCliente(p) {
    const email = String((p.payer && p.payer.email) || '').toLowerCase().trim();
    if (email) {
        const porEmail = clients.find(c => String(c.email || '').toLowerCase().trim() === email);
        if (porEmail) return { cliente: porEmail, motivo: 'e-mail confere' };
    }
    const valor = p.transaction_amount;
    const mesmoValor = clients.filter(c => Math.abs(getClientMonthlyValue(c) - valor) < 0.01);
    if (mesmoValor.length === 1) return { cliente: mesmoValor[0], motivo: 'valor do plano' };
    if (mesmoValor.length > 1) return { cliente: null, motivo: `${mesmoValor.length} planos com esse valor` };
    return { cliente: null, motivo: 'sem sugestão' };
}

async function mpBuscarPagamentos() {
    const corte = new Date();
    corte.setMonth(corte.getMonth() - MP_MESES);
    const corteISO = corte.toISOString().slice(0, 10);

    const todos = [];
    for (let offset = 0; offset < 2000; offset += 100) {
        const resp = await fetch(`${MP_WEBHOOK}?limit=100&offset=${offset}`);
        if (!resp.ok) throw new Error('Mercado Pago respondeu ' + resp.status);
        const data = await resp.json();
        const res = (data && data.results) || [];
        todos.push(...res);
        // A busca vem ordenada por data desc: para assim que passar da janela
...
async function mpCarregarRegistrados() {
    mpState.jaRegistrados = new Set();
    mpState.idsRegistrados = new Set();
    if (!db) return;
    const { data, error } = await db
        .from('pagamentos_clientes')
        .select('valor, data_pagamento, descricao');
    if (error) { console.warn('MP: não deu pra ler pagamentos_clientes:', error); return; }
    (data || []).forEach(p => {
        mpState.jaRegistrados.add(`${String(p.data_pagamento).slice(0, 10)}|${Number(p.valor).toFixed(2)}`);
        const m = /MP#(\d+)/.exec(p.descricao || '');
        if (m) mpState.idsRegistrados.add(m[1]);
    });
}

function mpEstaRegistrado(p) {
    if (mpState.idsRegistrados.has(String(p.id))) return true;
    return mpState.jaRegistrados.has(`${mpDataISO(p)}|${Number(p.transaction_amount).toFixed(2)}`);
}
...
function renderMercadoPago() {
    ...
    const pagos = mpState.pagamentos;
    const soma = arr => arr.reduce((s, p) => s + (p.transaction_amount || 0), 0);
    const extras = pagos.filter(p => mpCategoria(p) === 'extra');
    const assin = pagos.filter(p => mpCategoria(p) === 'assinatura');
    const avulsos = pagos.filter(p => ['link', 'pix'].includes(mpCategoria(p)));

    setText('mp-kpi-total', formatBRL(soma(pagos)));
    setText('mp-kpi-assin', `${formatBRL(soma(assin))} · ${assin.length}`);
    setText('mp-kpi-avulso', `${formatBRL(soma(avulsos))} · ${avulsos.length}`);
    setText('mp-kpi-extra', `${formatBRL(soma(extras))} · ${extras.length}`);

    const soPendentes = document.getElementById('mp-only-pending');
    const apenasPend = !soPendentes || soPendentes.checked;

    const linhas = pagos
        .filter(p => mpCategoria(p) !== 'extra')
        .filter(p => !apenasPend || !mpEstaRegistrado(p))
        .sort((a, b) => (mpDataISO(b) > mpDataISO(a) ? 1 : -1));

    if (!linhas.length) {
        list.innerHTML = `<div class="pay-empty">${apenasPend ? 'Tudo atribuído por aqui.' : 'Nenhum recebimento no período.'}</div>`;
        return;
    }

    const opcoes = [...clients]
        .sort((a, b) => String(a.company || a.name).localeCompare(String(b.company || b.name)))
        .map(c => `<option value="${esc(c.id)}">${esc(c.company || c.name)}</option>`)
        .join('');

    list.innerHTML = linhas.map(p => {
        const cat = mpCategoria(p);
        const rotulo = { assinatura: 'Assinatura', link: 'Link', pix: 'PIX' }[cat] || cat;
        const feito = mpEstaRegistrado(p);
        const sug = mpSugerirCliente(p);
        const email = (p.payer && p.payer.email) || '';
        const [y, m, d] = mpDataISO(p).split('-');
        return `
        <div class="mp-row ${feito ? 'is-done' : ''}" data-mp="${esc(p.id)}">
            <span class="mp-date">${d}/${m}/${y}</span>
            <span class="mp-value">${formatBRL(p.transaction_amount)}</span>
            <span><span class="mp-tag is-${cat}">${rotulo}</span></span>
            <span class="mp-desc">${esc(p.description || 'Sem descrição')}<small>${esc(email || sug.motivo)}</small></span>
            <span>
                <select data-mp-select="${esc(p.id)}" ${feito ? 'disabled' : ''}>
                    <option value="">— escolher cliente —</option>
                    ${opcoes}
                </select>
            </span>
            <span>${feito
                ? '<span class="mp-done-label"><i class="fas fa-check"></i> No histórico</span>'
                : `<button class="mp-assign" data-mp-assign="${esc(p.id)}">Registrar</button>`}</span>
        </div>`;
    }).join('');

    // Pré-seleciona a sugestão
    linhas.forEach(p => {
        const sel = list.querySelector(`[data-mp-select="${CSS.escape(String(p.id))}"]`);
        const sug = mpSugerirCliente(p);
        if (sel && sug.cliente) sel.value = String(sug.cliente.id);
    });
}

async function carregarMercadoPago() {
    const list = document.getElementById('mp-list');
    const btn = document.getElementById('btn-mp-refresh');
    if (!list || mpState.carregando) return;
    mpState.carregando = true;
    if (btn) btn.querySelector('i').classList.add('fa-spin');
    list.innerHTML = '<div class="pay-empty">Buscando no Mercado Pago...</div>';
    try {
        const [pagos] = await Promise.all([mpBuscarPagamentos(), mpCarregarRegistrados()]);
        mpState.pagamentos = pagos;
        renderMercadoPago();
    } catch (err) {
        console.error('Erro ao buscar Mercado Pago:', err);
        list.innerHTML = `<div class="pay-empty" style="color:var(--red)">Não deu pra buscar: ${esc(err.message || err)}</div>`;
    } finally {
        mpState.carregando = false;
        if (btn) btn.querySelector('i').classList.remove('fa-spin');
    }
}

async function mpAtribuir(mpId, storeId, btn) {
    const p = mpState.pagamentos.find(x => String(x.id) === String(mpId));
    if (!p || !db) return;
    if (!storeId) { alert('Escolha o cliente antes de registrar.'); return; }
    btn.disabled = true;
    btn.textContent = 'Salvando...';
    const data = mpDataISO(p);
    try {
        const { error } = await db.from('pagamentos_clientes').insert({
            store_id: storeId,
            tipo: 'mensalidade',
            ...
            data_pagamento: data,
            descricao: `MP#${p.id} · ${p.description || mpCategoria(p)}`
        });
        if (error) throw error;
        ...
        console.error('Erro ao registrar pagamento do MP:', err);
...
function setupMercadoPagoUI() {
    const btn = document.getElementById('btn-mp-refresh');
    if (btn) btn.addEventListener('click', carregarMercadoPago);

    const chk = document.getElementById('mp-only-pending');
    if (chk) chk.addEventListener('change', renderMercadoPago);

    const list = document.getElementById('mp-list');
    if (list) {
        list.addEventListener('click', e => {
            const b = e.target.closest('[data-mp-assign]');
            if (!b) return;
            const mpId = b.dataset.mpAssign;
            const sel = list.querySelector(`[data-mp-select="${CSS.escape(mpId)}"]`);
            mpAtribuir(mpId, sel ? sel.value : '', b);
        });
    }
}
